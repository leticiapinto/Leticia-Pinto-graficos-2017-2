...
namespace graphics
{
                  ...
                  const vector<vec2>& textures )
    {
        ...
        myIndexBuffer = new IndexBuffer();
        myIndexBuffer->setData( sizeof( Ind3 ) * indices.size(),  3 * indices.size(), (GLuint*) indices.data() );
    ...
    void Mesh::addTexture2D(vector<vec2>& myvertexTexture, GLuint index_) const
    {
        VertexBuffer* _tBuffer = new VertexBuffer();
        _tBuffer->setData( sizeof( vec2 ) * myvertexTexture.size(), 2, (GLfloat*) myvertexTexture.data() );
        myVertexArray->addBuffer( _tBuffer, index_ );
    }
}